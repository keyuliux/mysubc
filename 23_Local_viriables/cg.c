...
// Flag to say whhich section we are outputting in to
...
// Position of next local variable relative to stack base point
// We store the offset as positive to make aligning the stack pointer easier
static int localOffset;
static int stackOffset;

// Reset the position of new local variable when parsing a new function
...
// operation is pre- of post -increment/decrement,
  ...
  switch (Gsym[id].type) {
    ...
    case P_LONGPTR:
        ...
        if (op == A_POSTDEC)
          ...
          break;
    default:
        fatald("Bad type in cgloadlocal:", Gsym[id].type);
    }
    return (r);
}
...
// the number of the register with result
...
// Call printint() with the given reigster
...
int cgstorlocal(int r, int id) {
    ...
    {
        case P_CHAR:
            fprintf(Outfile, "\tmovb\t%s, %d(%%rbp)\n", breglist[r], Gsym[id].posn);
            break;
        case P_INT:
            fprintf(Outfile, "\tmovl\t%s, %d(%%rbp)\n", dreglist[r], Gsym[id].posn);
            break;
        case P_LONG:
        case P_CHARPTR:
        case P_INTPTR:
        case P_LONGPTR:
            fprintf(Outfile, "\tmovq\t%s, %d(%%rbp)\n", reglist[r], Gsym[id].posn);   
            break;
        default:
            fatald("Bad type in cgstorlocal:", Gsym[id].type);
    }
    return (r); 
}
...
void cgglobsym(int id)
{
    ...
    for (int i = 0; i < Gsym[id].size; i++) {
        switch (typesize)
        {
            ...
            default: fatald("Unknow typesize in cgglobsym: ", typesize);
        }
    }
